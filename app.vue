...
const overallRecommendation = computed(() => {
  ...
  return { title: "รอยยิ้มของคุณอยู่ในเกณฑ์ที่ดี", message: "ผลการวิเคราะห์โดยรวมของคุณอยู่ในเกณฑ์ดี หากต้องการให้รอยยิ้มสวยงามสมบูรณ์แบบในทุกมิติมากขึ้น สามารถนัดเข้ามาเพื่อปรึกษาและวางแผนการรักษาเฉพาะบุคคลได้เลย", cta: "สนใจนัดปรึกษาทันตแพทย์" };
});
...
// --- ฟังก์ชันใหม่: จัดการเมื่อผู้ใช้กดปุ่ม CTA ---
const handleCtaClick = async () => {
  if (!overallRecommendation.value || isLoading.value) return;

  // สร้างข้อความที่จะส่งแบบไดนามิก
  ...
  try {
    ...
    // ตรวจสอบว่าฟังก์ชัน sendMessages พร้อมใช้งานหรือไม่
    if (liff.isInClient() && liff.isApiAvailable('shareTargetPicker')) {
      ...
      // หลังจากส่งข้อความสำเร็จ ให้ปิดหน้าต่าง LIFF
      liff.closeWindow();
    } else {
      // สำหรับการทดสอบบน Desktop หรือกรณีที่ API ไม่พร้อมใช้งาน
      alert(`ถ้าเปิดในแอป LINE ข้อความนี้จะถูกส่งเข้าห้องแชท:\n\n${messageToSend}`);
    }
  } catch (error) {
